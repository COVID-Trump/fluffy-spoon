...
本仓库包含一套自动化工作流（GitHub Actions），用于持续反编译 Minecraft 各个版本，并将结果自动推送到一个**私有目标仓库**中。
...
*   **作用**: 授权工作流读取和写入**目标私有仓库**。默认的 `GITHUB_TOKEN` 无法跨仓库操作。
*   **如何获取**:
    1. 访问 GitHub [Personal access tokens (classic)](https://github.com/settings/tokens)。
    2. 点击 "Generate new token (classic)"。
    3. **注意范围 (Scopes)**: 必须勾选 **`repo`** (Full control of private repositories)。这是必须的，否则无法推送代码到私有仓库。
    4. 生成后复制 Token。
*   **配置**: 在本仓库的 Secrets 中新建名为 `GH_PAT` 的条目，粘贴 Token。

### 2. `TARGET_REPO` (目标仓库标识)
*(注：原需求中提到 TARGET_REPO_PATH 为路径，但通常仓库名更灵活。此处修正为配置目标仓库的全名，路径在工作流中固定)*
*   **更正**: 根据工作流设计，我们实际上需要配置的是**目标仓库的名称**。
...
1. **环境初始化**: 设置 Python 3.13 和 Java 25。
2. **状态检查**:
...
4. **循环处理**:
   ...
   - **提交**:
     ...
     - `git add`, `commit`, `tag` (标签格式: `mc-<version>`)。
5. **推送**: 将 commits 和 tags 推送到目标私有仓库。
